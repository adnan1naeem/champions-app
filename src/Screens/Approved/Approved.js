...
const Approved = () => {
  ...
  const renderItem = ({ item: listItem }) => (
    <View style={{ alignItems: 'center' }}>
      <TouchableOpacity style={styles.flatList_container} onPress={() => {
        ...
        <View style={{ flexDirection: 'row' }}>
          ...
          <View style={{ flex: 1, alignItems: 'flex-end', marginRight: "6%" }}>
            <View style={{ flexDirection: 'row' }}>
              <Text style={{ color: Colors.text_Color, fontSize: 20 }}>{listItem?.number}</Text>
    ...
    <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
      ...
      >
        ...
        <View style={styles.back_icon_view}>
          <Ionicons name="chevron-back" size={30} color={Colors.text_Color} onPress={() => {
            navigation.navigate('PaidCategory')
          }} />
          <View style={styles.filter_view}>
            <Text style={{ marginRight: "60%", color: Colors.text_Color, fontSize: 16, fontWeight: '500' }}>All
              <Entypo name="chevron-small-down" size={20} />
            </Text>
            <Text style={{ color: Colors.text_Color, fontSize: 16, fontWeight: '500' }}>Date
        ...
        <View style={{ marginTop: 10, alignItems: 'center', height: 170 }} >
          <Image style={{ height: '100%', width: "41%" }} source={require('../../Assets/Image/Orient_icon.png')} />
        </View>
        <View>
          <FlatList
            data={newScanData}
